...
# Streamlit app title
st.title("Dynamic Invoice Data Analyzer")
...
if uploaded_file is not None:
    ...
    if df['Invoice No'].dropna().empty:
        ...
        logger.info(f"Invoice No values (first 5): {df['Invoice No'].dropna().head().tolist()}")

    # Data cleaning options
    st.sidebar.header("Data Cleaning")
    fill_na = st.sidebar.checkbox("Fill missing numeric values with 0", value=True)
    if fill_na:
    ...
    def validate_calculations(df):
        try:
            ...
            # Tax Percentage Valid: Must be in [0, 5, 12, 18, 28] and error < 0.1
            ...
            # Final Amount Matches (despite invalid tax percentage)
    ...
    # Invoice Filters
    with st.sidebar.expander("Invoice Filters", expanded=True):
        if 'Invoice No' in df.columns:
            invoice_options = get_filter_options(df, 'Invoice No')
            if invoice_options:
                selected_invoices = st.multiselect("Invoice No", options=invoice_options, default=invoice_options,
                                                 help="Select one or more Invoice Numbers to filter.")
                logger.info(f"Invoice No options: {invoice_options[:5]}... (total {len(invoice_options)})")
                logger.info(f"Selected Invoice No: {selected_invoices[:5]}... (total {len(selected_invoices)})")
                if selected_invoices:
                    prev_len = len(df)
                    filtered_df = df[df['Invoice No'].isin([str(x) for x in selected_invoices]) | df['Invoice No'].isna()]
                    filter_debug.append(f"Invoice No filter: {prev_len} -> {len(filtered_df)} rows")
                    active_filters.append(f"Invoice No: {len(selected_invoices)} selected")
                    logger.info(f"Invoice No filter applied: {len(filtered_df)} rows remain")
                else:
                    filtered_df = df.copy()
                    filter_debug.append("Invoice No filter: Skipped (no selection)")
                    logger.info("Invoice No filter skipped: No selection")
            else:
                st.warning("No valid Invoice No values available for filtering.")
                filtered_df = df.copy()
                filter_debug.append("Invoice No filter: Skipped (no valid values)")
                logger.warning("No valid Invoice No values for filtering.")
        else:
            filtered_df = df.copy()
        ...
        if 'Bill Date' in df.columns and not df['Bill Date'].isna().all():
            ...
            if pd.notna(min_date) and pd.notna(max_date):
                date_range = st.date_input("Bill Date Range", [min_date, max_date], 
                                          ...
                                          help="Select a date range for Bill Date.")
                if len(date_range) == 2:
    ...
    with st.sidebar.expander("Categorical Filters", expanded=True):
        categorical_cols = ['Usage unit', 'Tax Percentage Valid', 'Quantity Valid', 'Final Amount Valid', 'Final Amount Matches', 'Tax Type', 'Customer Name', 'Item Name']
    ...
    if filtered_df.empty:
        ...
        st.write(f"Selected Invoice No (first 5): {selected_invoices[:5] if 'selected_invoices' in locals() else 'None'}")
    ...
    st.dataframe(display_df.style.apply(highlight_errors, axis=1))
    ...
    try:
        # Exclude total rows from visualizations
        viz_df = filtered_df[filtered_df['Row Type'] != 'Total']
    ...
    if 'Final Amount' in filtered_df.columns and not filtered_df.empty:
        ...
        if len(outliers) > 0:
            st.dataframe(outliers[['Invoice No', 'Item Name', 'Final Amount', 'Calculation Details']])
